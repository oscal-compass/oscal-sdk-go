...
func GenerateAssessmentResults(plan oscalTypes.AssessmentPlan, opts ...GenerateOption) (*oscalTypes.AssessmentResults, error) {
	...
	actorByCheck := make(map[string]string)
	if plan.AssessmentAssets != nil && plan.AssessmentAssets.Components != nil {
		for _, comp := range *plan.AssessmentAssets.Components {
			...
			for _, check := range checkProps {
				actorByCheck[check.Value] = comp.UUID
			}
		}
	}

	// Add initial origin values
	observationsByCheckTitle := make(map[string]oscalTypes.Observation)
	for _, observation := range options.observations {
		actor, found := actorByCheck[observation.Title]
		if found {
		...
		observationsByCheckTitle[observation.Title] = observation
	}
	...
	for _, task := range tasks {
		...
		for _, assocActivity := range *task.AssociatedActivities {
			...
			if activity.Steps != nil {
				...
				// Update the observations associated with an activity with an origin
				// TODO[jpower432] = Document the check ID is equal to the observation title.
				// Activity Title == Rule
				// One Observation per Activity Step
				// Observation Title == Check
				for _, step := range *activity.Steps {
					observation, ok := observationsByCheckTitle[step.Title]
					if !ok {
						continue
					}
